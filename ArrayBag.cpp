...
*/
template<class ItemType>
void ArrayBag<ItemType>::operator+=(const ArrayBag& rhs){
 for(int i = 0; i < rhs.item_count_; i++){items_.add(
   rhs.items_[i]);
   item_count_++;
 }
}
...
template<class ItemType>
void ArrayBag<ItemType>::operator/=(const ArrayBag& rhs){;
for(int i =0; i< rhs.item_count_; i++ ){if(!items_.contains(rhs.items_[i])){
    items_.add(rhs.items_[i]);
    item_count_++;
    }
   }
}
